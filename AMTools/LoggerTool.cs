...
namespace AMTools.Tools
{
    ...
    public class AMDevLogger : IAMLogger
    {
        ...
        [CallerMemberName] string caller = null)
        {
            rollingLog.Info($"{DateTime.UtcNow.ToString("O")} | Info | TID: {Thread.CurrentThread.ManagedThreadId} | {Path.GetFileNameWithoutExtension(path)} | {caller} () | Line: {lineNumber} | {message}");
        }
        ...
        public void LogError(string message, [CallerLineNumber] int lineNumber = 0, [CallerFilePath] string path = null,
            [CallerMemberName] string caller = null)
        {
            rollingLog.Info($"{DateTime.UtcNow.ToString("O")} | Error | TID: {Thread.CurrentThread.ManagedThreadId} | {Path.GetFileNameWithoutExtension(path)} | {caller} () | Line: {lineNumber} | {message}");
        }
    }
}
